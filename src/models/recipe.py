...
class Ingredient:
    ...
    def formatted_quantity(self) -> str:
        """Returns the quantity formatted with at most 1 decimal place"""
        if self.quantity == int(self.quantity):  # If it's a whole number
            return str(int(self.quantity))
        return f"{self.quantity:.1f}"  # One decimal place
